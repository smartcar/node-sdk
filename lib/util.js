...
util.request = function(options) {
  ...
  .catch(RequestError, StatusCodeError, function(e){
    switch(e.statusCode){
      ...
      case 409:
        throw new errors.StateError(options.uri + " " + options.form.action);
      case 429:
...
util.get = function(options) {
   ...
   * data in imperial units, otherwise defaults to metric units
...
util.action = function(options) {
   ...
   * @param  {Boolean} [apiOptions.imperial] set true to send imperial data
