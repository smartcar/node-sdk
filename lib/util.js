...
/**
 * Helper to extract error message from an StatusCodeError.
 */
const getErrorMessage = function(e) {
  return _.get(e, 'error.error_description') || _.get(e, 'error.message');
};
...
util.catch = function(error) {
  const options = error.options;
  switch (error.statusCode) {
    case 400:
      throw new errors.ValidationError(getErrorMessage(error));
    case 401:
    ...
    default:
      throw new errors.SmartcarError(error);
  }
};
