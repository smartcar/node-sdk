 ...
 * @property {Object[]} responses - The returned vehicle data.
 *
 * @example
 * {
 *   responses: [
 ...
 *       path: '/odometer',
 *       code: 200,
 *       headers: {'SC-Data-Age': '2019-09-10'},
 *       body: {
 *         distance: 32768
 *       },
 *    },
 ...
 * GET Vehicle.batch
 *
 * @param {BatchRequest} batchRequest - Contains all the endpoints and headers
 * for the batch request
 *
 * @return {Promise.<BatchRequest>} A promise for info on the vehicle's
 * battery status.
 * @throws {SmartcarError} - an instance of SmartcarError.
 *   See the [errors section](https://github.com/smartcar/node-sdk/tree/master/doc#errors)
 *   for all possible errors.
 */
Vehicle.prototype.batch = function(batchRequest) {
  const {requests, defaultHeaders} = batchRequest;
  const req = this.request.post('batch', {
    body: {requests, headers: defaultHeaders},
  });
  return util.wrap(req);
};
