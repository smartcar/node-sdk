...
Vehicle.prototype.disconnect = function() {
  return util.wrap(this.request.delete('application')).then((res) => {
...
Vehicle.prototype.permissions = function() {
  return util
    .wrap(this.request.get('permissions'))
    .then((response) => response.body.permissions);
};
...
Vehicle.prototype.info = function() {
  return util.wrap(this.request.get('/')).then((res) => {
    const response = {...res.body};
...
Vehicle.prototype.location = function() {
  const req = this.request.get('location');
  return util.wrap(req).then((res) => {
    const response = {
      data: res.body,
      age: parseAge(res),
    };
...
Vehicle.prototype.odometer = function() {
  const req = this.request.get('odometer');
  return util.wrap(req).then((res) => {
    const response = {
      data: res.body,
      age: parseAge(res),
      unitSystem: res.headers['sc-unit-system'],
    };
...
Vehicle.prototype.vin = function() {
  return util.wrap(this.request.get('vin')).then((response) => response.body.vin);
};
 ...
 * @throws {SmartcarError} on unsuccessful request
 */
Vehicle.prototype.lock = function() {
  const req = this.request.post('security', {
    body: {action: 'LOCK'},
  });
  return util.wrap(req).then((res) => {
    const response = {
