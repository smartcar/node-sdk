 ...
 *
 * @return {Promise.<BatteryCapacity>} A promise for info on the vehicle's battery capacity.
 * @throws {SmartcarError} - an instance of SmartcarError.
 *   See the [errors section](https://github.com/smartcar/node-sdk/tree/master/doc#errors)
 *   for all possible errors.
 */
Vehicle.prototype.batteryCapacity = function() {
  const req = this.request.get('battery/capacity', {resolveWithFullResponse: true});
  return util.wrap(req).then(function(response) {
    return {
      data: response.body,
      age: parseAge(response),
 ...
 * POST Vehicle.lock
 *
 * @return {Promise.<ActionSuccess>} response on successful request
 ...
 *   for all possible errors.
 */
Vehicle.prototype.startCharge = function() {
  const req = this.request.post('charge', {body: {action: 'START'}});
  return util.wrap(req).then(({status}) => ({status}));
};
 ...
 *   for all possible errors.
 */
Vehicle.prototype.stopCharge = function() {
  const req = this.request.post('charge', {body: {action: 'STOP'}});
  return util.wrap(req).then(({status}) => ({status}));
};
 ...
 * @type {Object}
 * @typedef Batch
 * @property {Object} responses - The object containing multiple HTTP responses.
 * @property {Object} data.ENDPOINT - The HTTP response for a given endpoint.
 * ENDPOINT is a Smartcar endpoint (i.e. /odometer, /fuel).
 * @property {Number} data.ENDPOINT.code - The HTTP status code for this response.
