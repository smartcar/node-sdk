 ...
 *
 * @param {String[]|String} permissions Permission(s) to check if the vehicle has
 *
 * @return {Promise.<Boolean>} An array of permissions names.
 */
Vehicle.prototype.hasPermissions = function(permissions) {
  return this.permissions()
    .then(function(vehiclePermissions) {
      if (_.isArray(permissions)) {
        return _.difference(permissions, vehiclePermissions).length === 0;
      } else {
