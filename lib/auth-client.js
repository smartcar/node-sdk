 ...
 * @param {String[]} [options.flags] - List of feature flags that your application
 * has early access to.
...
AuthClient.prototype.getAuthUrl = function(options) {
  ...
  if (options.flags) {
    parameters.flags = options.flags.join(' ');
  }

  const mode = this.development || this.testMode;
 ...
 * @param {String} - an optional country code according to [ISO 3166-1 alpha-2](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2). Defaults to 'US'.
 ...
 *   for all possible errors.
 */
AuthClient.prototype.isCompatible = function(vin, scope, country = 'US') {
  const qs = {
    vin,
    scope: scope.join(' '),
    country,
