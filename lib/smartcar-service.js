...
SmartcarService.prototype.request = async function(
  ...
  const shouldOmitBody =
    type.toLowerCase() === 'get' || Object.keys(body).length === 0;
  ...
  const response = await util.wrap(
    shouldOmitBody
      ? this.requestObject.defaults(requestConfig)[type](path)
      : this.requestObject.defaults(requestConfig)[type](path, {...body}),
  );
