...
test('exchangeCode - without flags', async function(t) {
  const client = new AuthClient({
    clientId: CLIENT_ID,
    clientSecret: CLIENT_SECRET,
    redirectUri: 'https://insurance.co/callback',
...
test('exchangeRefreshToken - without flags', async function(t) {
  const client = new AuthClient({
    clientId: CLIENT_ID,
    clientSecret: CLIENT_SECRET,
    redirectUri: 'https://insurance.co/callback',
...
test('getAuthUrl - single select with invalid enabled value', function(t) {
  const client = new AuthClient({
    clientId: CLIENT_ID,
    clientSecret: CLIENT_SECRET,
    redirectUri: 'https://insurance.co/callback',
  ...
  const actual = client.getAuthUrl(['read_odometer', 'read_vehicle_info'], {
    state: 'fakestate',
    forcePrompt: true,
    singleSelect: {enabled: 'invalid'}, // neither true, false, nor vin
  });
