...
var json = require('../lib/vehicle_methods');
var vehicleMethodBlacklist = require('./vehicle_method_blacklist');
...
suite('Vehicle prototype', function() {
  ...
  suiteSetup(function() {
    ...
    for (var method in Vehicle.prototype) {
      if (Vehicle.prototype.hasOwnProperty(method)
       && !vehicleMethodBlacklist[method]) {
        methods.push(method);
      }
    }
    Object.keys(json).forEach(function(method) {
      var config = json[method];
  ...
  // });
  //
  test('all prototype methods', function() {
    return Promise.each(methods, function(method) {
      return vehicle[method]()
      .then(function(response) {
        expect(response).to.have.all.keys('status');
        expect(response.status).to.equal('success');
